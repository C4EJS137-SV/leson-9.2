...
// Nhập vào 1 chuỗi keyword. 
//Hãy lấy ra email của các user có tên (first_name + last_name) chứa keyword.
// let keyword = prompt('nhập vào 1 chuỗi key work ' )
...
console.log(` số lượng người trên 50  ${countAge}`);
...
for (const user of users) {
    if (user.is_married) {
        count++;
    } else if (user.is_married !== count) {
        countOne++;
        // console.log(user);
    }
};
console.log(` số lượng kết hôn ${count}`);
console.log(` số lượng chưa kết hôn ${countOne}`);
